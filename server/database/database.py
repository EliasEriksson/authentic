...
class Database:
    ...
    _engine: AsyncEngine
    _reader_maker: async_sessionmaker[AsyncSession]
    _writer_maker: async_sessionmaker[AsyncSession]

    def __init__(self, engine: AsyncEngine) -> None:
        self._configuration = Configuration()
        self._engine = engine
        self._reader_maker = async_sessionmaker(
            bind=engine,
            expire_on_commit=False,
        )
        self._writer_maker = async_sessionmaker(
            bind=engine,
            expire_on_commit=False,
            autobegin=False,
        )
    ...
    async def client(self) -> AsyncIterator[Client]:
        async with self._reader_maker() as reader:
            async with self._writer_maker() as writer:
